function generateGoogleTTSLink(q, speed, tl, tkk) {
    var tk = calcHash(q, tkk);
    var url = 'http://translate.google.com/translate_tts?ie=UTF-8&total=1&idx=0&client=t&ttsspeed=${speed}&tl=${tl}&tk=${tk}&q=${q}&textlen=${q.length}';
    url = url.replace("${tl}", tl);
    url = url.replace("${tk}", tk);
    url = url.replace("${q}", q);
    url = url.replace("${speed}", speed);
    url = url.replace("${q.length}", q.length);
